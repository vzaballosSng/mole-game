...
export class GamePage extends LitElement {
	...
	_startGame() {
		this._timerId = setInterval(
			() => this._showMoleAlt(),
			TIME_LEVEL[this._levelSelected]
		);
	...
	_showMoleAlt() {
		this._currentTileSelected = this._moleManager.getRandomTile(
			TILES_NUMBER,
			MOLES_NUMBER,
			this._previousTileSelected
		);
		if (this._previousTileSelected || this._previousTileSelected.length) {
			this._previousTileSelected.forEach((numberTile) => {
				this._tilesAltArray[numberTile].show = false;
				this._tilesAltArray[numberTile].status = MOLE_STATUS.dead;
			});
		}

		this._previousTileSelected = [...this._currentTileSelected];
		this._currentTileSelected.forEach((numberTile) => {
			this._tilesAltArray[numberTile].show = true;
			this._tilesAltArray[numberTile].status = MOLE_STATUS.alive;
		});
	...
	_clickMole(ev) {
		const { position, showMole } = ev.target;
		if (
			showMole &&
			this._tilesAltArray[position].status === MOLE_STATUS.alive
		) {
			this._savePoints();
			this._tilesAltArray[position].status = MOLE_STATUS.dead;
	...
	render() {
		return html`
			...
			<div class="button-container">
				<custom-button data-state=${this._state} @click=${this._handleButton}
